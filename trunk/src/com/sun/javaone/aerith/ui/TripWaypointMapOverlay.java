...
import org.jdesktop.swingx.mapviewer.animation.EvaluatorGeoPosition;
...
final public class TripWaypointMapOverlay extends
		WaypointMapOverlay<JXMapViewer> implements WaypointRenderer {
	...
	public Set<Waypoint> getWaypoints() {
		return new HashSet<Waypoint>(this.tripEditPanel.getTrip()
				.getWaypoints());
	}

	public Animator createMapMoveAnimation(GeoPosition newPosition) {
		KeyValues<GeoPosition> keyValues = KeyValues.create(
				new EvaluatorGeoPosition(), this.tripEditPanel.getMapViewer()
						.getCenterPosition(), newPosition);
		KeyFrames keyFrames = new KeyFrames(keyValues);
		Animator mapAnimator = PropertySetter.createAnimator(400,
				this.tripEditPanel.getMapViewer(), "centerPosition", keyFrames);
	...
	public void mouseClicked(MouseEvent evt) {
		if (SwingUtilities.isMiddleMouseButton(evt)
				|| (SwingUtilities.isLeftMouseButton(evt) && evt
						.getClickCount() == 2)) {
			Rectangle bounds = this.tripEditPanel.getMapViewer()
					.getViewportBounds();
			double x = bounds.getX() + evt.getX();
			double y = bounds.getY() + evt.getY();
			int zoom = this.tripEditPanel.getMapViewer().getZoom();
			GeoPosition geopos = tripEditPanel.getMapViewer().getTileFactory()
					.pixelToGeo(new Point2D.Double(x, y), zoom);
			createMapMoveAnimation(geopos).start();
			// createMapMoveAnimation(GoogleUtil.getPosition(new
			// Point2D.Double(x, y), zoom)).start();
			...
			if (wp != null) {
				...
				mapAnimator.addTarget(new TimingTarget() {
					...
					public void end() {
						...
						AnimationUtil.createFadeInAnimation(waypointEditPanel)
								.start();
					}
			...
			// must adjust for the viewport.... this is lame and should be in
			// JXMapViewer, I think
			...
			// GeoPosition p = GoogleUtil.getPosition(new Point2D.Double(x, y),
			// tripEditPanel.mapViewer.getZoom());

			GeoPosition p = tripEditPanel.getMapViewer().getTileFactory()
					.pixelToGeo(new Point2D.Double(x, y),
							tripEditPanel.mapViewer.getZoom());

			this.tripEditPanel.getTrip().addWaypoint(
					new com.sun.javaone.aerith.model.Trip.Waypoint(p));
	...
	private void startRolloverAnimation() {
		...
		enterController = PropertySetter.createAnimator(400, this,
				"selectedFade", getSelectedFade(), 0.99f);
	...
	private void stopRolloverAnimation() {
		...
		exitController = PropertySetter.createAnimator(400, this,
				"selectedFade", getSelectedFade(), 0.0f);
	...
	private Trip.Waypoint findWaypoint(final Point screenPoint) {
		for (Trip.Waypoint w : this.tripEditPanel.getTrip().getWaypoints()) {
			// Point2D point = GoogleUtil.getBitmapCoordinate(w.getPosition(),
			// tripEditPanel.mapViewer.getZoom());
			Point2D point = tripEditPanel.getMapViewer().getTileFactory()
					.getBitmapCoordinate(w.getPosition(),
							tripEditPanel.mapViewer.getZoom());
	...
	public boolean paintWaypoint(Graphics2D g, JXMapViewer map,
		...
		// String number =
		// Integer.toString(tripEditPanel.getTrip().getWaypoints().indexOf(waypoint)+1);
		Font oldFont = g.getFont();
		g.setFont(g.getFont().deriveFont(Font.BOLD, 14f));

		g.setColor(FONT_COLOR);

		// TextLayout tl = new TextLayout(number, g.getFont(),
		// g.getFontRenderContext());
		// Rectangle2D textBounds = tl.getBounds();
		// int xx = x-10+(int)(20-textBounds.getWidth())/2;
		// int yy = y-10+(int)(20-textBounds.getHeight())/2 +
		// (int)textBounds.getHeight();
		// g.drawRect(xx, yy, (int)textBounds.getWidth(),
		// (int)textBounds.getHeight());
	...
	protected void paintWaypointSummary(Graphics2D g, JXMapViewer map,
		...
		JComponent dummy = new JXPanel();// exists soley to pass to the
											// painter
	...
	private class PhotoImportHandler extends TransferHandler {
		...
		private Animator createAnimator(Component glassPane) {
			Animator hoverAnimator = PropertySetter.createAnimator(300,
					glassPane, "zoom", 1.0f, .85f);
		...
		public boolean canImport(TransferHandler.TransferSupport support) {
			...
			if (wp != null && hoverAnimator == null) {
				hoverAnimator = createAnimator(SwingUtilities.getRootPane(
						tripEditPanel).getGlassPane());
				hoverAnimator.start();
			} else if (wp == null && hoverAnimator != null) {
				hoverAnimator.stop();
				((GhostGlassPane) SwingUtilities.getRootPane(tripEditPanel)
						.getGlassPane()).setZoom(1.0f);
				hoverAnimator = null;
			}
		...
		public boolean importData(TransferHandler.TransferSupport support) {
			try {
				...
				for (DataFlavor flavor : support.getDataFlavors()) {
					if (PhotoWrapperTransferable.FLICKR_FLAVOR.equals(flavor)) {
						PhotoWrapper photo = (PhotoWrapper) support
								.getTransferable().getTransferData(flavor);
						Trip.Waypoint wp = findWaypoint(support
